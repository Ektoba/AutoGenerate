...
class ChangeHandler(FileSystemEventHandler):
    ...
    def run_update_script(self):
        ...
        try:
            ...
            # 실제로 삭제된 파일들의 디렉토리를 기억하기 위한 set
            deleted_dirs = set()

            if not files_to_delete:
                self.logger.info("삭제할 파일이 없습니다. 프로젝트 파일만 갱신합니다.")
            ...
            # 파일 삭제 후, 해당 폴더들이 비었는지 체크해서 정리
            for dir_path in deleted_dirs:
                try:
                    if not os.listdir(dir_path):
                        self.logger.info(f"빈 폴더 발견! 함께 정리합니다 (휴지통으로 이동): {dir_path}")
                        self.file_deleter.delete_folder(dir_path)  # FileDeleter에 폴더 삭제 기능 추가 필요
